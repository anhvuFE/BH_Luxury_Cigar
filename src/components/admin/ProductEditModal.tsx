...
const ProductEditModal: React.FC<ProductEditModalProps> = ({
  ...
  useEffect(() => {
    if (product && isOpen) {
      setFormData({
        ...
        specifications: typeof product.specifications === 'string' ? product.specifications : (product.specifications ? JSON.stringify(product.specifications) : ''),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!productIdentifier) {
      setErrors({ general: 'Không xác định được sản phẩm để cập nhật.' });
      return;
    }
    ...
    try {
      ...
      const productData: Partial<Product> = {
        ...
        specifications: (() => {
          const value = formData.specifications.trim();
      ...
      if (uploadedImagePath) {
        ...
        productData.images = [uploadedImagePath];
      }

      // Update product
      const updatedProduct = await adminService.updateProduct(productIdentifier, productData);

      onProductUpdated(updatedProduct);
      onClose();
    } catch (error) {
      console.error('Error updating product:', error);
      setErrors({ general: 'Không thể cập nhật sản phẩm. Vui lòng thử lại.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Chỉnh sửa sản phẩm" size="2xl">
      <div className="p-6">
